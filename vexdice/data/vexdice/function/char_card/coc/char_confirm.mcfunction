...
tellraw @s [{text:"[确认] ",click_event:{action:"run_command",command:"/trigger vex.char.settings set 20"},color:"green"}]
tellraw @s [{text:"==========",color:dark_gray},{text:"================",color:dark_gray},{text:"==========",color:dark_gray}]

execute if score @s vex.char.LOCK matches 0 run tellraw @s [{text:"角色卡已锁定",color: "dark_green"}]
